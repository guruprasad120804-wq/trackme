...
export default function LoginPage() {
  ...
  const handleGoogleAuth = (mode: "signup" | "login") => {
    setLoading(mode);
    const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
    const redirectUri = `${window.location.origin}/auth/callback`;
    const scope = "openid email profile";
    // signup: force consent screen; login: just account picker
    const prompt = mode === "signup" ? "consent" : "select_account";
    const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=${scope}&access_type=offline&prompt=${prompt}`;
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-amber/30 bg-amber/5 p-5">
              <p className="text-xs font-medium text-amber mb-1">New to TrackMe?</p>
              <p className="text-[11px] text-muted-foreground mb-3">Create your free account with Google</p>
              <Button
                onClick={() => handleGoogleAuth("signup")}
                disabled={loading !== null}
                size="lg"
                className="w-full bg-amber hover:bg-amber-dark text-navy font-semibold h-11"
              >
                <GoogleIcon />
                {loading === "signup" ? "Redirecting..." : "Sign Up with Google"}
              </Button>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-xs font-medium text-foreground mb-1">Already have an account?</p>
              <p className="text-[11px] text-muted-foreground mb-3">Sign in to access your portfolio</p>
              <Button
                onClick={() => handleGoogleAuth("login")}
                disabled={loading !== null}
                size="lg"
                className="w-full bg-white hover:bg-gray-100 text-gray-900 font-semibold border border-gray-300 h-11"
              >
                <GoogleIcon />
                {loading === "login" ? "Redirecting..." : "Login with Google"}
              </Button>
            </div>
          </div>
